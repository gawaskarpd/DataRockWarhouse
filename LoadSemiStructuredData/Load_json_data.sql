...
--TRY TO FIND THE CITYNAME & YEAR LIVED

SELECT F.value:cityName::string as city_name,
count(*) as year_lived 
FROM json_demo, 
TABLE(FLATTEN(v:citiesLived))f
group by 1;



SELECT 
F.value:cityName::string as city_name,
array_size(f.value:yearsLived) as year_lived
FROM json_demo,    -- 1
TABLE(FLATTEN(v:citiesLived))f,  --5
TABLE(FLATTEN(f.value:yearsLived)) yl;

--find out the cities lived in 1998

Select
F.value:cityName::string as city_name,
count(f.value:cityName::string) as year_lived
from json_demo,    -- 1
TABLE(FLATTEN(v:citiesLived))f,  --5
TABLE(FLATTEN(f.value:yearsLived)) yl
where yl.value::string = '2024'
Group by 1;


SELECT 
F.value:cityName::string as city_name,
array_size(v:citiesLived[0].yearsLived) as year_lived
FROM json_demo,    -- 1
TABLE(FLATTEN(v:citiesLived))f,  --5
TABLE(FLATTEN(f.value:yearsLived)) yl;


select V,f.value,YL.VALUE
from json_demo,
TABLE(FLATTEN(v:citiesLived))f,
TABLE(FLATTEN(f.value:yearsLived)) yl;
